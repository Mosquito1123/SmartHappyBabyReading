...
@property (nonatomic, strong) LLCardViewBindHelper *bindHelper;
...
-(void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    ...
    if (self.status == CardViewStatusEdit) {
        ...
        self.cardArray = [[BBDataBaseManager sharedInstance] selectAllDataFromTable:@"mine"];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info{
    ...
    if (self.originalImage) {
        ...
        BBKardObject *oldCard = [self.cardArray objectAtIndex:self.indexToDelete];
                ...
                [NetworkManager translate2English:card.chinese ok:^(NSString *english, NSError *error) {
                    if (error) {
                        ...
                        if (self.status == CardViewStatusEdit) {
                            ...
                            [[BBDataBaseManager sharedInstance] deleteDataFromTable:@"mine" card:oldCard];
                            [[BBDataBaseManager sharedInstance] insertDataIntoTable:@"mine" card:card];
                        }
                        [self.cardCollectionView reloadData];
...
- (void)setUpSubviews {
    ...
    [self.editButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id sender) {
        ...
        [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
            ...
            textField.text = [(BBKardObject *)[self.cardArray objectAtIndex:self.indexToDelete] chinese];
        }];
    ...
    [self.saveButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id sender) {
        if (self.buttonType == SaveButtonTypeDidSave) {
            [[BBDataBaseManager sharedInstance] deleteDataFromTable:@"mine" card:[self.cardArray objectAtIndex:self.indexToDelete]];
            self.buttonType = SaveButtonTypeNotSave;
        } else if (self.buttonType == SaveButtonTypeNotSave) {
            [[BBDataBaseManager sharedInstance] insertDataIntoTable:@"mine" card:[self.cardArray objectAtIndex:self.indexToDelete]];
            self.buttonType = SaveButtonTypeDidSave;
            if (self.status == CardViewStatusCustom) {
                self.status = CardViewStatusEdit;
                self.cardArray = [NSArray arrayWithObjects:[[[BBDataBaseManager sharedInstance] selectAllDataFromTable:@"mine"] lastObject], nil];
            }
        }
    }];
...
- (LLCardViewBindHelper *)bindHelper {
    if (_bindHelper == nil) {
        _bindHelper = [[LLCardViewBindHelper alloc] init];
    }
    return _bindHelper;
}
