...
- (UICollectionViewCell *)collectionView:(UICollectionView *)cv cellForItemAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    if ([self.identifier isEqualToString:@"mine"]) {
        ...
        cell.textLabel.text = [[(BBKardObject *)self.cardArray[indexPath.row] chinese] substringToIndex:1];
    } else {
...
- (void)didClickDeleteButton:(CardListCollectionViewCell *)cell {
    ...
    UIAlertAction *deleteAction = [UIAlertAction actionWithTitle:@"是" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        BBKardObject *card = [self.cardArray objectAtIndex:index];
        [[BBDataBaseManager sharedInstance] deleteDataFromTable:@"mine" card:card];
...
- (void)setIdentifier:(NSString *)identifier {
    ...
    if ([identifier isEqualToString:@"mine"]) {
        _cardArray = [[BBDataBaseManager sharedInstance] selectAllDataFromTable:identifier];
    } else {
        _cardArray = [[DDJsonHelper sharedInstance] getCardArrayWithIdentifier:identifier];
    }
}
