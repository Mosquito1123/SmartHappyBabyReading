...
{
    if ([YTConfig sharedInstance].secretId.length <= 0 || [YTConfig sharedInstance].secretKey.length <= 0) {
        NSLog(@"ERROR: secretId & secretKey empty!");
        return nil;
    }
    if ([YTConfig sharedInstance].userId.length > USER_ID_MAX_LEN) {
    ...
    NSString *origin = [NSString stringWithFormat:@"a=%@&k=%@&e=%u&t=%u&r=%d&u=%zd&f=%@", [YTConfig sharedInstance].appId, [YTConfig sharedInstance].secretId, expired + now, now, rdm, [[YTConfig sharedInstance].userId integerValue], @""];
    NSData *data = [self hmacsha1:origin secret:[YTConfig sharedInstance].secretKey];
